...
{
	int rc, expected_count = folio_expected_ref_count(src) + 1;
	void* dst_private_data = dst->private;
	...
	if (mode == MIGRATE_NO_COPY) {
		/*
		 * Store dst_private_data in src->private as src is no longer
		 * used after metadata copy and this avoids extra allocation
		 */
		src->private = dst_private_data;
	}
...
{
	struct folio *folio, *folio2, *dst, *dst2;
	int rc, nr_pages, nr_batched_folios = 0;
	int old_page_state = 0;
	struct anon_vma *anon_vma = NULL;
	int is_thp;
	LIST_HEAD(err_src);
	...
	list_for_each_entry_safe(folio, folio2, src_folios, lru) {
		...
		if (unlikely(page_movable_ops(&folio->page)))
			...
			rc = migrate_folio_content(dst, folio, MIGRATE_NO_COPY);
		/*
		 * The rules are:
	...
	if (static_call(_should_handle)(src_folios, nr_batched_folios))
	...
	list_for_each_entry_safe(folio, folio2, src_folios, lru) {
		...
		migrate_folio_finalize_move(dst, folio, 0);

		/*
		 * Below few steps are only applicable for lru pages which is
		 * ensured as we have removed the non-lru pages from our list.
		 */
		migrate_folio_remap(folio, dst, old_page_state);

		migrate_folio_complete(folio, dst, reason, anon_vma);
